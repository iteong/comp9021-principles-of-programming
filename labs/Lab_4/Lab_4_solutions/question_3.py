...
def get_names(problem_size, gender, default_name):
    ...
    while True:
        try:
            individuals = input('Enter {0:} names for the {1:}, all on one line and separated by spaces,\n'
                                '  or just press Enter for the default "names" {2:}_1, ..., {2:}_{0:}: '
                                .format(problem_size, gender, default_name)).split()
...
def get_preferences(problem_size, men, women):
    user_choice = input('\nPress Enter to get a default preference for all men or women.\n'
                        ...
                        '  to be prompted and enter the preferences of your choice: ').split()
    if user_choice:
...
def get_random_preferences(problem_size, gender_1, gender_2, gender_1_is_male):
    ...
    for individual in gender_1:
        ...
        print('')
        record_preferences(problem_size, individual, gender_1_preferences, gender_1_is_male, preferences)
...
def get_preferences_from_user(problem_size, gender_1, gender_2, gender_1_is_male):
    ...
    for individual in gender_1:
        while True:
            try:
                preferences = input('List preferences for {:}, in decreasing order: '.format(individual)).split()
...
def record_preferences(problem_size, individual, gender_preferences, gender_is_male, preferences):
    if gender_is_male:
        # We reverse each man's list of preferences, so chosing the preferred woman is effectively achieved
        # by popping (the last element) from that list.
        gender_preferences[individual] = [preferences[i] for i in range(problem_size - 1, -1, -1)]
    else:
        # For women, it is convenient to explictily record each man's rank in her preferences
        # rather than implicitly via the index of a list.
...
def determine_stable_matching(men_preferences, women_preferences):
    free_men = set(men_preferences.keys())
