...
class GameOfLife(Tk):
    def __init__(self):
        ...
        help_menu.add_command(label = 'The "Start/Stop evolution" button', command = self.evolution_button_help)
        ...
        self.evolution_button = Button(buttons, text = 'Start evolution', width = 13, command = self.evolve)
    ...
    def evolution_button_help(self):
        tkinter.messagebox.showinfo('The "Start/Stop evolution" button',
            'This button allows one to apply the rules of the game '
            'for as long as wished, and to stop it once it has been started.\n\n'
...
class Board(Frame):
    def __init__(self):
        ...
        self.board = Canvas(self, width = DIM * CELL_SIZE + 5, height = DIM * CELL_SIZE + 5, bg = BOARD_COLOUR)
        self.switch = False
        self.draw_board()
        self.board.pack()
    ...
    def create_gosper_glider_gun(self):
        ...
        cells = self.cells[self.switch]
        cells[26][2] = True
        cells[24][3] = True; cells[26][3] = True
        cells[14][4] = True; cells[15][4] = True; cells[22][4] = True; cells[23][4] = True; cells[36][4] = True; cells[37][4] = True
        cells[13][5] = True; cells[17][5] = True; cells[22][5] = True; cells[23][5] = True; cells[36][5] = True; cells[37][5] = True
        cells[2][6] = True; cells[3][6] = True; cells[12][6] = True; cells[18][6] = True; cells[22][6] = True; cells[23][6] = True
        cells[2][7] = True; cells[3][7] = True; cells[12][7] = True; cells[16][7] = True; \
            cells[18][7] = True; cells[19][7] = True; cells[24][7] = True; cells[26][7] = True
    ...
    def draw_board(self):
        for i in range(DIM + 1):
            self.board.create_line(5, 5 + CELL_SIZE * i, 5 + CELL_SIZE * DIM, 5 + CELL_SIZE * i, width = 0.5, fill = LINE_COLOUR)
            self.board.create_line(5 + CELL_SIZE * i, 5, 5 + CELL_SIZE * i, 5 + CELL_SIZE * DIM, width = 0.5, fill = LINE_COLOUR)
    ...
    def flip_cell(self, event):
        if not self.master.evolving and 5 <= event.x <= CELL_SIZE * DIM + 5 and 5 <= event.y <= CELL_SIZE * DIM + 5:
            i = int(floor((self.board.canvasx(event.x) - 5) / CELL_SIZE))
            j = int(floor((self.board.canvasx(event.y) - 5) / CELL_SIZE))
            self.board.delete(self.drawn_cells[i][j])
