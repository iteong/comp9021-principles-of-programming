...
class KMeansClustering(Tk):
    def __init__(self):
        ...
        help_menu.add_command(label = 'Creating points and initial centroids', command = self.creating_points_and_initial_centroids_help)
        self.config(menu = menubar)

        self.space = Space()
        buttons = Frame(bd = 20)        
        self.configure_space_or_cluster_button = Button(buttons, text = 'Cluster', width = 5, command = self.configure_space_or_cluster)
        self.configure_space_or_cluster_button.pack(padx = 30, side = LEFT)
        self.clear_or_iterate_button = Button(buttons, text = 'Clear', width = 5, command = self.clear_or_iterate)
    ...
    def principle_help(self):
        tkinter.messagebox.showinfo('Principle',
            ...
            'create k (square) points, or "centroids", displayed in different colors.\n'
            'Clicking the button displaying "Iterate" gives each point the colour '
            'of the closest centroid, making that point a member of the cluster '
            'associated with that colour.\n\n'
            'The centre of gravity of each cluster then becomes the new centroid. '
            'The same computation can be done again by clicking the button displaying '
            '"Iterate", until the clusters do not change any more, in which case '
            'the button labels change and the user is in a position to run another experiment.\n\n'
            'The user can also click the button displaying "Stop" to get back to that position, '
            'and he or she change his or her mind by clicking again on the button displaying "Cluster".')
    ...
    def clearing_help(self):
        tkinter.messagebox.showinfo('Clearing',
            ...
            'In case no centroid is displayed, possibly because the "Clear" button '
            'has just been clicked, then clicking the "Clear" button deletes '
            'all points.')
    ...
    def creating_points_and_initial_centroids_help(self):
        tkinter.messagebox.showinfo('Creating points and initial centroids',
            ...
            'No point or centroid is created when it is too close to an existing point '
            'or centroid, respectively.\n\n'
...
class Space(Frame):
    ...
    def clear(self):
        if self.centroids:
            for centroid_coordinates in self.centroids:
                self.space.itemconfig(self.centroids[centroid_coordinates].drawn_point, fill = '', outline = '')
            self.centroids.clear()
            for point_coordinates in self.points:
                self.points[point_coordinates].colour = POINT_COLOUR
                self.space.itemconfig(self.points[point_coordinates].drawn_point, fill = POINT_COLOUR, outline = POINT_COLOUR)
            self.available_colours = list(self.colours)
        else:
            for point_coordinates in self.points:
                self.space.itemconfig(self.points[point_coordinates].drawn_point, fill = '', outline = '')
            self.points.clear()
    ...
    def act_on_click(self, event):
        ...
        coordinates = (x, y)
        if self.clustering:
            if self.request_point_otherwise_delete_or_ignore(coordinates, self.centroids, 8) and self.available_colours:
            ...
            if self.request_point_otherwise_delete_or_ignore(coordinates, self.points, 25):
                self.points[coordinates] = Point(self.space.create_oval(x - 2, y - 2, x + 2, y + 2, fill = POINT_COLOUR, outline = POINT_COLOUR),
                                                 POINT_COLOUR)
    ...
    def request_point_otherwise_delete_or_ignore(self, coordinates, points, size):
        for point_coordinates in points:
            if self.square_of_distance(coordinates, point_coordinates) < size:
                ...
                return False
        for point_coordinates in points:
            if self.square_of_distance(coordinates, point_coordinates) < 4 * size:
                return False
        return True
    ...
    def iterate(self):
        clusters = dict((centroid_coordinates, []) for centroid_coordinates in list(self.centroids))
        if not clusters:
            return
        ...
        for point_coordinates in self.points:
            ...
            if self.points[point_coordinates].colour != colour:
                ...
                self.space.itemconfig(self.points[point_coordinates].drawn_point, fill = colour, outline = colour)
        ...
        for centroid_coordinates in clusters:
            ...
            if nb_of_points:
                x = sum(coordinates[0] for coordinates in clusters[centroid_coordinates])
                y = sum(coordinates[1] for coordinates in clusters[centroid_coordinates])
